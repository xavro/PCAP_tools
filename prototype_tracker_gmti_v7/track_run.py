# -*- coding: utf-8 -*-
"""Noyau d'exécution du tracker v7 SANS matplotlib — réutilisable (GUI, CLI, tests).
...
v7 : mécanismes grande zone (DELETE_SEC, CONFIRM_BY_HITS, GATE_GROW_MPS) et
profil `routier_zone`. Dépendances : numpy + scipy (via `tracker`). Pas de matplotlib.
"""
import csv
import itertools
...
# Profils de tuning par environnement (source de vérité — alignée sur demo.py v7).
PROFILES = {
    "defaut":    {},
    ...
    # surveillance grande zone (scan sectoriel) : vie de piste en secondes,
    # confirmation par cumul de hits, gate croissant avec l'ancienneté.
...
def csv_dwells(path):
    """Lit le CSV de plots, groupe par dwell, projette en plan local ENU."""
    rows = []
...
def run_tracking(path, profile="defaut", overrides=None):
    """Décode le CSV et déroule le tracker. Retourne des données Python pures
    prêtes à dessiner :
      {raw:[(x,y)], tracks:[{id,hits,pts,smooth}], n_kept, n_rejected, frame}
    """
