# -*- coding: utf-8 -*-
"""Noyau d'exécution du tracker SANS matplotlib — réutilisable (GUI, CLI, tests).

Sépare le CALCUL (décodage CSV -> pistes) du RENDU : `demo.py` s'en sert pour la
partie compute (et ajoute matplotlib pour ses figures) ; la console Tkinter
`pcap_console.py` s'en sert pour dessiner les pistes sur un Canvas natif (aucune
dépendance matplotlib).

Dépendances : numpy + scipy (via `tracker`). PAS de matplotlib.
"""
import csv
import itertools
...
# Profils de tuning par environnement (source de vérité unique — cf. README v5).
PROFILES = {
    "defaut":    {},
...
# Instantané des défauts de Params, pour repartir propre à chaque run (le profil
# mute la classe globale ; sans reset, deux runs successifs s'empileraient).
_DEFAULTS = {k: getattr(T.Params, k) for k in dir(T.Params) if k.isupper()}


def apply_profile(name, overrides=None):
    """Repart des défauts, applique le profil `name`, puis d'éventuels overrides."""
...
def csv_dwells(path):
    """Lit le CSV de plots (13 colonnes), groupe par dwell, projette en plan local ENU."""
    rows = []
...
def run_tracking(path, profile="defaut", overrides=None):
    """Décode le CSV et déroule le tracker. Retourne des données en Python pur
    (aucun objet numpy exposé), prêtes à dessiner :

      {raw:[(x,y)], tracks:[{id,hits,pts:[(x,y)],smooth:[(x,y)]}],
       n_kept, n_rejected, frame}
    """
    apply_profile(profile, overrides)
    T.Track._ids = itertools.count(1)      # IDs repartent à 1 à chaque run
    tk = T.Tracker()
    raw = []
    frame = None
