...
(function () {
  ...
  // La SOURCE du fond de carte est réglée dans « Paramètres » (panneau d'environnement), à côté du
  // catalogue de services qu'elle utilise : deux formulaires pour un seul fichier laissaient l'opérateur
  // sans savoir lequel fait foi. Ne reste ici que la case d'affichage, propre à la carte regardée.
  // Le panneau d'environnement rappelle cette fonction après un enregistrement, pour que la carte suive
  // sans recharger la page.
  window.stxBasemapReload = async () => {
    try { state.bmCfg = await api("/api/basemap"); applyBasemap(); } catch { /* fond inchangé */ }
  };
  $("btn-settings").addEventListener("click", () => { const st = $("settings"); st.hidden = !st.hidden; if (!st.hidden) renderFilesInfo(); });
  ...
  function renderFilesInfo() {
    ...
    $("st-files").innerHTML = `profils tracker : <b>${(gs.prof && gs.prof.path) || "gmti_profiles.json"}</b><br>` +
      `dernier pcap : <b>${state.pcap || "—"}</b><br>fond de carte : basemap.json (Paramètres) · réglages : pcap_web_settings.json` +
...
MISB ST 0601 · tag ${f.tag}${std ? " — " + std : ""}`;
  ...
  function clipAt(ev) {                                                       // clip sous le pointeur (lane « clips »)
    ...
    if (ev.offsetY < LN.c.y - 1 || ev.offsetY > LN.c.y + LN.c.h + 1) return null;
    const t = (ev.offsetX / tl.clientWidth) * D + (pb.tl.t0 || 0);
    return clipsOf().find(c => t >= c.start_utc && t <= c.end_utc) || null;
  }
  ...
  function drawTimeline() {
    ...
    if (LN.c && pb.on && pb.tl) {                            // clips extraits (violet) — clic : aller au début, clic droit : menu
      const t0 = pb.tl.t0 || 0;
