...
def csv_dwells(path):
    ...
    def fget(r, key, default, sentinel=None):
        v = r.get(key, "")
        if v in ("", None):
            return default
        x = float(v)
        # convention 4607 : champ "tout a 1" = no statement -> valeur par defaut
        return default if (sentinel is not None and x == sentinel) else x
    ...
    for key in sorted(dwells, key=lambda k: min(int(r["dwell_time_ms"]) for r in dwells[k])):
        ...
        for r in grp:
            ...
            # hygiene : plot a plus de 400 km du capteur = coordonnees aberrantes
            if r.get("sensor_lat"):
                sx0, sy0 = frame.to_xy(float(r["sensor_lat"]), float(r["sensor_lon"]))
                if math.hypot(x - sx0, y - sy0) > 400_000:
                    continue
            sig_r = fget(r, "sig_range_cm", Params.R_POS_DEFAULT * 100, sentinel=65535) / 100.0
            sig_x = fget(r, "sig_xrange_dm", Params.R_POS_DEFAULT * 10, sentinel=65535) / 10.0
            R = None
            ...
            plots.append(Plot(x, y, r_pos=max(sig_r, sig_x), R=R,
                              vel_los=fget(r, "vel_los_cms", 0, sentinel=32767) / 100.0,
...
def run(source, png="tracker_result.png", csv_out="tracks_out.csv"):
    ...
    with open(csv_out, "w", newline="") as f:
        ...
        w.writerow(["track_id", "t_s", "x_m", "y_m", "etat", "aerien", "rotateur"])
        for tr in kept:
            for (t, x, y, st, hit) in tr.trajectory():
                w.writerow([tr.id, f"{t:.1f}", f"{x:.1f}", f"{y:.1f}", st,
                            int(tr.is_air and not tr.is_rotator), int(tr.is_rotator)])
...
PROFILES = {
    ...
    # surveillance grande zone (scan sectoriel) : une cible n'est PAS vue a chaque
    # dwell -> vie de piste en SECONDES, confirmation par cumul de hits,
    # gate qui grandit avec l'anciennete de la derniere mise a jour
